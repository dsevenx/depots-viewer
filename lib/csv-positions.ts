...
/**
 * Parses CSV and returns array of positions (without id, bankId, and createdAt)
 */
export function parsePositionCSV(
  ...
  return rows.map((row, index) => {
    ...
    if (!row.isin?.trim()) {
      throw new Error(`Zeile ${index + 2}: ISIN ist ein Pflichtfeld`);
    }
    if (!row.ticker?.trim()) {
      throw new Error(`Zeile ${index + 2}: Ticker ist ein Pflichtfeld`);
    }
    if (!row.assetType?.trim()) {
      throw new Error(`Zeile ${index + 2}: Asset-Typ ist ein Pflichtfeld`);
    }
    if (!row.purchaseDate?.trim()) {
      throw new Error(`Zeile ${index + 2}: Kaufdatum ist ein Pflichtfeld`);
    }
    if (!row.quantity?.trim()) {
      throw new Error(`Zeile ${index + 2}: Anzahl ist ein Pflichtfeld`);
    }
    if (!row.purchasePrice?.trim()) {
      throw new Error(`Zeile ${index + 2}: Kaufpreis ist ein Pflichtfeld`);
    }
    if (!row.currency?.trim()) {
      throw new Error(`Zeile ${index + 2}: Währung ist ein Pflichtfeld`);
    }
    ...
    if (!['stock', 'etf', 'bond'].includes(assetType)) {
      throw new Error(
        `Zeile ${index + 2}: Asset-Typ muss 'stock', 'etf' oder 'bond' sein`
      );
    }
    ...
    if (!['EUR', 'USD'].includes(currency)) {
      throw new Error(`Zeile ${index + 2}: Währung muss 'EUR' oder 'USD' sein`);
    }

    // Parse numbers
    const quantity = parseFloat(row.quantity.trim());
    if (isNaN(quantity) || quantity <= 0) {
      throw new Error(`Zeile ${index + 2}: Anzahl muss eine positive Zahl sein`);
    }

    const purchasePrice = parseFloat(row.purchasePrice.trim());
    if (isNaN(purchasePrice) || purchasePrice <= 0) {
      throw new Error(
        `Zeile ${index + 2}: Kaufpreis muss eine positive Zahl sein`
      );
    }
    ...
    if (isNaN(purchaseDate.getTime())) {
      throw new Error(
        `Zeile ${index + 2}: Kaufdatum hat ungültiges Format (erwartetes Format: YYYY-MM-DD)`
      );
    }
